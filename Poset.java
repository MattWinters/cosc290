...
public class Poset {
    ...
    private Map<Integer, List<Integer>> hasse;
    private Map<Integer, List<Integer>> invhasse;
    ...
    public Map<Integer, List<Integer>> invert(){
      Map<Integer, List<Integer>> invH = new HashMap<>();
      for (int i : hasse.keySet()){
        invH.put(i, new LinkedList<>());
      }

      for (int i : hasse.keySet()){
        List<Integer> nbrs = hasse.get(i);
        for(int j : nbrs){
          List<Integer> invIndex = invH.get(j);
          invIndex.add(i);
        }
      }
      return invH;
    ...
    public Set<Integer> maximal() {
      Set<Integer> maximals = new HashSet<Integer>();
        for (int i : hasse.keySet()){
          if((hasse.get(i)).length() == 0){
            maximals.add(i);
          }
        }
        return maximals;
    ...
    public void printInverse(){
      for (int i : invhasse.keySet()){
        System.out.print(i + " - ");
        List<Integer> nbrs = invhasse.get(i);
        for(int j : nbrs){
          System.out.print(j + ",");
        }
        System.out.println();
    ...
    public static void main(String[] args) {
        ...
        poset.printPoset();
        //poset.printInverse();
        System.out.println(poset.maximals());
    }
